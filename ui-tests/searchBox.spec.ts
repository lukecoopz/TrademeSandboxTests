...
test.describe("Search Box Tests", () => {
  ...
  const searchResults =
    'div[class="tm-search-results__container ng-star-inserted"]'
  ...
  test("should return results for a valid search term", async ({ page }) => {
    await page.fill(searchInput, "cable")
  ...
  test("should show 0 results for an uncommon search term", async ({
    page,
  }) => {
    ...
    await expect(
      page.getByText("Showing 0 results for 'unicorn'")
    ).toBeVisible()
  })
  ...
  test("should show suggestions based on input", async ({ page }) => {
    ...
    await expect(page.locator(suggestionsBox).first()).toBeVisible({
      timeout: 3000,
    })
  ...
  test("should handle search with special characters", async ({ page }) => {
    ...
    // Verify the search page shows expected results
    await expect(page.getByText("Showing 2 results for 'S9+'")).toBeVisible({
      timeout: 3000,
    }) //again not best way as search results may vary in test
  })
  ...
  test("should not allow SQL injection", async ({ page }) => {
    ...
    await expect(page.getByText("Showing 0 results for 'test")).toBeVisible({
      timeout: 3000,
    })
